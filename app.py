...
# -----------------------------------------------------------------------------
    ...
    with tabs[0]:
        ...
        with col2:
            st.markdown('<p style="color:#2b8cbe; font-weight:bold; font-size:18px;"> Conteo de valores nulos</p>', unsafe_allow_html=True)
            st.dataframe(df.isna().sum().to_frame("null_count"), width=300,height=400)          
